...
namespace FMS.Server.Controllers.Transaction
{
    ...
    public class PurchaseReturnController(IPurchaseReturnSvcs purchaseReturnSvcs, UserManager<AppUser> userManager) : ControllerBase
    {
        ...
        public async Task<IActionResult> Create([FromBody] PurchaseReturnOrderModel model)
        {
        ...
        public async Task<IActionResult> Update([FromRoute] Guid id, [FromBody] PurchaseReturnOrderModel model)
        {
            if (id != Guid.Empty)
            {
                if (ModelState.IsValid)
