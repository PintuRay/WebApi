...
namespace FMS.Server.Controllers.Transaction
{
    ...
    public class DamageController(IDamageSvcs damageSvcs, UserManager<AppUser> userManager) : ControllerBase
    {
        ...
        public async Task<IActionResult> Create([FromBody] DamageOrderModel model)
        {
        ...
        public async Task<IActionResult> Update([FromRoute] Guid id, [FromBody] DamageOrderModel model)
        {
            if (id != Guid.Empty)
            {
                if (ModelState.IsValid)
