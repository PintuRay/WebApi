...
namespace FMS.Server.Controllers.User
{
    ...
    public class SubLedgerBalanceController(ISubLedgerBalanceSvcs subLedgerBalanceSvcs, UserManager<AppUser> userManager) : ControllerBase
    {
        ...
        public async Task<IActionResult> Create([FromBody] SubLedgerBalanceModel model)
        {
        ...
        public async Task<IActionResult> Update([FromRoute] Guid id, [FromBody] SubLedgerBalanceModel model)
        {
            if (id != Guid.Empty)
            {
                if (ModelState.IsValid)
